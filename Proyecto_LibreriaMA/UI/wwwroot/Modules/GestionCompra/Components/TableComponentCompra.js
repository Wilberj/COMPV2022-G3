...
class TableComponentCompra extends HTMLElement {
    constructor(config = {
        Dataset: [],
        Functions: [],
        ModelObject: {}
    ...
    DrawTableComponent = async (Dataset = this.config.Dataset) => {
        ...
        Dataset.forEach((data, index) => {
            ...
            if (this.config.Functions) {
                if (index == 0) {
                    this.thead.append(Render.Create({
                        tagName: "th", innerHTML: "Descuento"
                    }));
                }
...
customElements.define('w-tablecompra', TableComponentCompra);
export { TableComponentCompra }